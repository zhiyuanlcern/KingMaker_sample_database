...
def filter_df(df, channel):
    ...
    selection_dic["mm"] = {"pt_1":"> 15", "pt_2":"> 15","mt_1" : "> 0",  }
    for i in selection_dic[channel]:
        filter_condition = []
        for var in df.GetColumnNames():
            if str(var).startswith(i):
...
def post_proc(f, samples_list, keep_only_nom=False, era='2022postEE', channel='mt', cut=0, out_vars =[], pnn_format= False):
    ...
    if not ".root" in f or "roota.root" in f:
    ...
    {'genWeight': '1.0f', 'Xsec' : '1.0f', 'is_fake' : 'false', "gen_match_1": 'int(-1)',"gen_match_2": 'int(-1)', "puweight": "double(1.0)", "btag_weight" : "double(1.0)", 
    ...
    "ZPtMassReweightWeight" : "double(1.0)", "FF_weight": "double(1.0)",
    ...
    df_mc, _2 = Add_new_column(df_mc, 
                                {'Train_weight': weight_dict[channel],}, overwrite=False)
    ...
    pnn_remove = ["id_tau_vsJet_VVVLoose_2","genWeight_tmp","id_tau_vsJet_VVLoose_2","FF_weight_tmp", "FF_weight_tmp1", "FF_weight_tmp2" ,"id_tau_vsJet_VLoose_2", "id_tau_vsJet_Loose_2", "id_wgt_tau_vsJet_Tight_2","id_tau_vsJet_Tight_2","id_wgt_tau_vsJet_VTight_2","id_wgt_tau_vsJet_VTight_2", "id_wgt_tau_vsJet_VVTight_2",
        "gen_pdgid_1", "gen_pdgid_2","gen_taujet_pt_2","genbosonmass", "genbosonpt", "tau_gen_match_2","pt_ll" ,"mass_ll", "PNN"]
