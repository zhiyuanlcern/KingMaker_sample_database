...
# for 2023preBPix era (Run3Summer23NanoAODv12)
...
OUTPUT_DIR="$REPO_DIR/2023/bbh_htautau"
...
echo "Generating BBH YAML files for 2023preBPix (Run3Summer23NanoAODv12)..."
echo "Output directory: $OUTPUT_DIR"

# Extract BBH samples for 2023preBPix (Summer23NanoAODv12, era: 2023)
...
for sample_name, sample_data in datasets.items():
    if 'BBHto2Tau' in sample_name and 'Run3Summer23NanoAODv12_private' in sample_name and 'BPix' not in sample_name:
        ...
        # era can be either an int or string, check both
        if era == 2023 or str(era) == '2023':
...
print(f"\nTotal BBH samples generated for 2023preBPix: {count}")
EOF
